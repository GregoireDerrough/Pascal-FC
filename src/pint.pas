...
program pint(objfile, pmdfile, input, output);
  ...
  var
    ...
    ir: objorder;
    ps: (
      run, fin, divchk, inxchk, charchk,
      stkchk, redchk, deadlock, channerror, guardchk,
      queuechk, procnchk, statchk, nexistchk, namechk,
      casechk, bndchk, instchk, inpchk, setchk,
      ovchk, seminitchk
    );
  ...
  procedure nameobj(target: integer; var tp: types; var tofile: text);
    ...
    procedure unselector(var rf: index; var tp: types);
      ...
      begin
        write(tofile, '[');
        with objfile^.genatab[rf] do begin
    ...
    (* name monitor boundary queue, h-p queue or any variable declared in a monitor *)
    ...
    begin
      with objfile^ do begin
        ...
        else if offset = 1 then begin  (* h-p queue *)
          printname(gentab[tptr].name, tofile);
          write(tofile, ' (monitor high-priority queue)')
        end
        else begin  (* declared variable *)
          write(tofile, '.');
  ...
  begin  (* Nameobj *)
    if target > ptab[0].stacksize then begin
  ...
  begin
    ...
    with ptab[nproc] do begin
      ...
      if termstate or (suspend <> 0) then begin
        writeln(pmdfile);
        writeln(pmdfile, 'Process suspended on:');
        writeln(pmdfile);
        if suspend > 0 then begin
          nameobj(suspend, tp, pmdfile);
          printyp(tp, pmdfile)
        end
        else begin
          frameptr := chans;
          for loop := 1 to abs(suspend) do begin
            chanptr := s[frameptr].i;
            if chanptr <> 0 then begin  (* 0 means timeout *)
  ...
  begin
    ...
    with objfile^ do begin
      ...
      while gentab[h1].link <> 0 do
        with gentab[h1] do begin
          if obj = variable then
            if typ in (stantyps + [semafors, enums]) then begin
              noglobals := false;
              case typ of
                ints, semafors, enums: writeln(pmdfile, name, ' = ', s[taddr].i);
                reals: writeln(pmdfile, name, ' = ', s[taddr].r);
                bools: writeln(pmdfile, name, ' = ', itob(s[taddr].i));
                chars: writeln(pmdfile, name, ' = ', chr(s[taddr].i mod 64));
              end   (* case *)
            end;  (* if *)
  ...
  procedure runprog;
    ...
    begin
      ...
      with eventqueue do begin
        frontpointer := first;
        if frontpointer <> nil then begin
          backpointer := nil;
          foundplace := false;
        ...
        if frontpointer = first then begin
          first := thisnode;
          time := waketime
        end
      end  (* with eventqueue *)
    ...
    begin
      ...
      repeat
        ...
        while not foundproc and (d >= 0) do
          with ptab[procindex] do begin
            foundproc := active and (suspend = 0) and (wakeup = 0) and not termstate;
            if not foundproc then begin
        ...
        if not foundproc then
          if procwaiting then
            if eventqueue.first <> nil then begin
    ...
    begin
      with ptab[pnum] do begin
        nchans := abs(suspend);
        frameptr := chans;
        for loop := 1 to nchans do begin
          chanptr := s[frameptr].i;
          if chanptr <> 0 then begin  (* timeout if 0 *)
            s[chanptr].i := 0;
            if chanptr = h then
              if onselect then begin
    ...
    begin
      ...
      with eventqueue do begin
        frontpointer := first;
        while (frontpointer <> nil) and not finished do begin
          with ptab[frontpointer^.proc] do begin
    ...
    begin
      if s[curmon+1].i > 0 then
        procwake(curmon+1)
      else if s[curmon].i > 0 then begin
        procwake(curmon);
    ...
    begin
      ...
      if not eof then
        if input^ in ['0'..'9'] then begin
          ...
          if input^ = '.' then begin  (* fractional part *)
            get(input);
            repeat
              if input^ in ['0'..'9'] then begin
          ...
          else if input^ in ['e', 'E'] then begin
            readscale(e, numerror);
            if e <> 0 then
              adjustscale(rnum, k, e, numerror)
          end
    ...
    repeat
      ...
      with ptab[curpr] do
        case ir.f of
          ...
          8: case ir.y of
            ...
            3: (* real sqr *)
              if (realmax / abs(s[t].r)) < abs(s[t].r) then
                ...
                s[t].r := sqr(s[t].r);
            4: s[t].i := btoi(odd(s[t].i));
            5: if not (s[t].i in [charl..charh]) then
              ps := charchk;
            6:;
            7: s[t].i := s[t].i + 1; (* succ *)
            8: s[t].i := s[t].i - 1; (* pred *)
            9: (* round *)
            ...
            24: begin (* int - bitset to integer *)
              h1 := 0;
          ...
          12: (* case1 *)
            if s[t].i = s[t - 1].i then begin
              t := t - 2;
              pc := ir.y
            end
            else
              t := t - 1;
          13: ps := casechk; (* case 2 *)
          14: begin (*for1up*)
            h1 := s[t - 1].i;
            if h1 <= s[t].i then
              s[s[t - 2].i].i := h1
          ...
          18: begin
            if ir.x = 1 then begin (* process *)
              if npr = pmax then begin
                ps := procnchk;
                goto 98
              end
              else begin
          ...
          19: begin
            ...
            b := h1;
            t := h4;
            pc := objfile^.gentab[h2].taddr
          end;
          21: with objfile^ do begin (*index*)
            h1 := ir.y; (*h1 points to genatab*)
            h2 := genatab[h1].low;
            h3 := s[t].i;
            if h3 < h2 then
              ps := inxchk
            else if h3 > genatab[h1].high then
              ps := inxchk
            else begin
              t := t - 1;
          ...
          27: begin (*read*)
            if eof(input) then
              ...
              case ir.y of
                ...
                4: readreal(s[s[t].i].r) (* real *)
              end;

            t := t - 1
          end;
          28: begin (*write string*)
            if ir.x = 1 then begin
              h3 := s[t].i;
              t := t - 1
            end
            else
          ...
          30: begin (* write formatted *)
            h3 := s[t].i;  (* field width *)
            t := t - 1;
            case ir.y of
              1: write(s[t].i:h3);  (* ints *)
              2: write(itob(s[t].i):h3);  (* bools *)
              3:
                if (s[t].i < charl) or (s[t].i > charh) then
                  ps := charchk
                else
